...
| Field | Value |
|---|---|
| Package status | `1.4.0_stable` -- reusable stable package, not L5 archival/publication-ready. |
...
| Intended package trajectory | `1.x_stable`; `L5 archival/publication-ready` is reserved for a later release with independent external review, preservation closure, registry publication, and L5 release approval |
...
This repository uses a controlled status progression: `0.1.0_public_draft`, `1.0.0-rc.N`, and `1.x_stable`. The current release is prepared as the stable v1.4.0 framework with L4 reusable-stable package conformance.

L5 archival/publication-ready status is not claimed for v1.4.0. L5 requires a separate release decision with completed preservation closure, registry publication, independent external review, archival fixity evidence, and L5 release approval.
...
9. **Assessment rubrics** for structured DSR review, rubric-guided evaluation, trace-quality assessment, and AI-assisted ingestion.

## Minimal L1/L2 pathway

The full DSR Framework repository remains an L4 reusable-stable package, but downstream users do not always need to adopt the full L4 file structure. The minimal L1/L2 pathway provides a low-burden lane for small artifacts, early-stage studies, coursework, dissertation planning, prereview packets, and internal lab records.

Use [docs/protocol/l1-l2-minimal-documentation-pathway.md](docs/protocol/l1-l2-minimal-documentation-pathway.md) when the goal is either `l1_documented` orientation or `l2_reviewable` documentation. The pathway includes L1 and L2 YAML templates, a README template, checklist, JSON Schema, example package, and a lightweight validator:
